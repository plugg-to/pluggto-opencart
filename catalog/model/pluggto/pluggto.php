...
class ModelPluggtoPluggto extends Model{
  ...
  public function getProduct($product_id){
      $url = "https://api.plugg.to/skus/".$product_id;
      $method = "get";
      $accesstoken = $this->getAccesstoken();
      $params = array("access_token" => $accesstoken);
      $data = $this->sendRequest($method, $url, $params);    
  ...
  public function getManufacturerID($brand) {
    ...
    public function getProductsStore($data = array()) {
        ...
        if($data['status']){
            ...
            $sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = ".$data['status']." AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
        } else {
        ...
        foreach ($query->rows as $result) {
            $product_data[$result['product_id']] = $this->getProductStore($result['product_id']);
        }
